...
def get_data(api_connection, api_host, api_port, path, params={}):
    # Extract the device ID and iBaseToken from the connection response
    device_id = api_connection[0]['data']['deviceid']
    ...
    # Set headers, including the iBaseToken for authentication
    headers = {
        'iBaseToken': iBaseToken,
        'Content-Type': 'application/json'
    }

    # Make the GET request with headers, cookies, and any additional parameters
    r = requests.get(endpoint, verify=False, headers=headers, cookies=api_cookies, params=params)

    # Decode the response (check encoding, 'utf-8' is more common)
